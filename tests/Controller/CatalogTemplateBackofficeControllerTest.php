...
final class CatalogTemplateBackofficeControllerTest extends WebTestCase
{
    ...
    public function testCatalogTemplateMutationsRequireCsrfToken(): void
    {
        ...
        self::assertSame(0, (int) $this->connection()->fetchOne('SELECT COUNT(*) FROM catalog_templates'));
    }
    ...
    public function testGlobalTemplateAdminCanCreateAndConfigureCatalogTemplate(): void
    {
        ...
        $this->post($client, '/app/modeles-catalogue', [
        ...
        $this->post($client, '/app/modeles-catalogue/'.$templateId.'/catalogue/ranks', [
            'role_key' => 'Comète',
        ...
        $this->post($client, '/app/modeles-catalogue/'.$templateId.'/catalogue/stats', [
            'name' => 'Éther',
        ]);
        ...
        $this->post($client, '/app/modeles-catalogue/'.$templateId.'/catalogue/welcome-messages', [
            'rank_id' => (string) $rankId,
        ...
        $this->post($client, '/app/modeles-catalogue/'.$templateId.'/catalogue/bye-messages', [
            'rank_id' => (string) $rankId,
    ...
    public function testIncompleteTemplateCannotBePublished(): void
    {
        ...
        $templateId = $this->seedEmptyTemplate();

        $this->post($client, '/app/modeles-catalogue/'.$templateId.'/publication', [
            'published' => '1',
        ]);

        self::assertResponseRedirects('/app/modeles-catalogue/'.$templateId.'/configuration');
        self::assertSame(0, (int) $this->connection()->fetchOne('SELECT published FROM catalog_templates WHERE id = ?', [$templateId]));
    ...
    private function seedEmptyTemplate(): int
    {
        $this->connection()->insert('catalog_templates', [
